...
const authHead = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Lookout</title><style>` + authCSS + `</style></head><body>`
const authFoot = `</body></html>`

func authTmpl(body string) *template.Template {
	return template.Must(template.New("t").Parse(authHead + body + authFoot))
}
...
<div class="wrap wide"><div class="card">
  <div class="bar"><div class="logo" style="margin:0">Look<b>out</b> &middot; Users</div><div class="row"><a href="/account">Account</a><a href="/">Dashboard</a></div></div>
  {{if .Err}}<div class="err">{{.Err}}</div>{{end}}
  <table>
    <tr><th>Email</th><th>Name</th><th>Role</th><th>MFA</th><th>Status</th><th></th></tr>
    {{range .Users}}
    <tr>
